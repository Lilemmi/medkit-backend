...
import { getFamilyMemberById, updateFamilyMember } from "../../../../src/services/family.service";
...
export default function EditFamilyScreen() {
  ...
  async function loadMember() {
    ...
    try {
      setLoadingData(true);
      const member = await getFamilyMemberById(Number(id));
      
      if (member && typeof member === 'object' && 'name' in member) {
        setName((member as any).name || "");
        setRole((member as any).role || "");
        setBirthDate((member as any).birthDate || (member as any).birthdate || "");
        setGender((member as any).gender || "");
        setAllergies((member as any).allergies || "");
        setPhotoUri((member as any).photoUri || null);
        
        setWeight((member as any).weight ? (member as any).weight.toString() : "");
        setHeight((member as any).height ? (member as any).height.toString() : "");
        setChronicDiseases(
          (member as any).chronicDiseases 
            ? (Array.isArray((member as any).chronicDiseases) ? (member as any).chronicDiseases.join(", ") : String((member as any).chronicDiseases))
            : ""
        );
        setMedicalConditions(
          (member as any).medicalConditions 
            ? (Array.isArray((member as any).medicalConditions) ? (member as any).medicalConditions.join(", ") : String((member as any).medicalConditions))
            : ""
        );
        setOrganConditions(
          (member as any).organConditions 
            ? (Array.isArray((member as any).organConditions) ? (member as any).organConditions.join(", ") : String((member as any).organConditions))
            : ""
        );
      }
    } catch (error) {
      console.error("Error loading family member:", error);
      Alert.alert(t("common.error"), "Не удалось загрузить данные");
    } finally {
  ...
  async function save() {
    ...
    try {
      await updateFamilyMember(Number(id), {
        name: name.trim(),
        role: role.trim() || null,
      ...
      Alert.alert(t("common.success"), "Данные обновлены", [
        {
          text: t("common.ok"),
          onPress: () => router.back(),
        },
      ...
      Alert.alert(t("common.error"), "Не удалось обновить данные");
    } finally {
      setLoading(false);
    }
  }
