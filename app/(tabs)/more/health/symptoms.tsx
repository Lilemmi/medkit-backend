...
export default function SymptomsScreen() {
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("symptom", user?.id);
  ...
  const handleSave = async () => {
    if (!symptom.trim()) {
      Alert.alert(t("common.error"), t("health.symptomDescribe"));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.symptoms")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.symptomDescribe")}</Text>
        <TextInput
          style={styles.input}
          placeholder={t("health.symptomPlaceholder")}
          placeholderTextColor={colors.textSecondary}
          value={symptom}
          onChangeText={setSymptom}
        />

        <Text style={styles.label}>{t("health.symptomNotes")}</Text>
        <TextInput
          style={[styles.input, styles.textArea]}
          placeholder={t("health.symptomNotesPlaceholder")}
          placeholderTextColor={colors.textSecondary}
          value={notes}
