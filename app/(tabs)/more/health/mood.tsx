...
export default function MoodScreen() {
  ...
  const MOOD_OPTIONS = [
    { id: "excellent", label: t("health.moodExcellent"), emoji: "😄", value: 5 },
    { id: "good", label: t("health.moodGood"), emoji: "🙂", value: 4 },
    { id: "normal", label: t("health.moodNormal"), emoji: "😐", value: 3 },
    { id: "bad", label: t("health.moodBad"), emoji: "😔", value: 2 },
    { id: "terrible", label: t("health.moodTerrible"), emoji: "😢", value: 1 },
  ];
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("mood", user?.id);
  ...
  const handleSave = async () => {
    if (selectedMood === null) {
      Alert.alert(t("common.error"), t("health.moodSelect"));
      return;
    }
  ...
  const getMoodLabel = (value: number) => {
    ...
    return mood ? `${mood.emoji} ${mood.label}` : value.toString();
  };
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.mood")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.moodSelect")}</Text>
      ...
      <Text style={styles.sectionTitle}>{t("health.moodHistory")}</Text>
      {records.length === 0 ? (
