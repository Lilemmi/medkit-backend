...
export default function DoctorVisitsScreen() {
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("doctor_visit", user?.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveHealthMetric({
        ...
        value: 0,
        unit: "",
        notes: `${t("health.doctorName")}: ${doctor}\n${t("health.visitDate")}: ${visitDate}\n${reason ? `${t("health.visitReason")}: ${reason}\n` : ""}${notes || ""}`,
        date: visitDate,
      });
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.doctorVisits")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.doctorName")}</Text>
        <TextInput
          style={styles.input}
        ...
        <Text style={styles.label}>{t("health.visitDate")}</Text>
        <TextInput
          style={styles.input}
          placeholder={t("health.visitDatePlaceholder")}
          placeholderTextColor={colors.textSecondary}
          value={date}
          onChangeText={setDate}
        />

        <Text style={styles.label}>{t("health.visitReason")}</Text>
        <TextInput
          style={styles.input}
        ...
        <TextInput
          ...
          placeholder={t("health.visitNotesPlaceholder")}
          placeholderTextColor={colors.textSecondary}
          value={notes}
      ...
      <Text style={styles.sectionTitle}>{t("health.visitHistory")}</Text>
      {records.length === 0 ? (
            ...
            <View style={styles.recordHeader}>
              <View style={{ flex: 1 }}>
                <Text style={styles.recordValue}>
                  {record.notes?.split("\n")[0]?.replace(`${t("health.doctorName")}: `, "") || t("health.doctorVisits")}
                </Text>
