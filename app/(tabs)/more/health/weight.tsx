...
export default function WeightScreen() {
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("weight", user?.id);
  ...
  const handleSave = async () => {
    if (!weight) {
      Alert.alert(t("common.error"), t("health.enterWeight"));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.weight")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.weight")} ({t("health.weightUnit")})</Text>
        <TextInput
          style={styles.input}
          placeholder="70"
