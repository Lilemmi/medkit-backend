...
export default function PulseScreen() {
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("pulse", user?.id);
  ...
  const handleSave = async () => {
    if (!pulse) {
      Alert.alert(t("common.error"), t("health.enterPulse"));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.pulse")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.pulseValue")} ({t("health.unit")})</Text>
        <TextInput
          style={styles.input}
          placeholder="70"
