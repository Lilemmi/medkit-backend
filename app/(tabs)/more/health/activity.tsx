...
export default function ActivityScreen() {
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("activity", user?.id);
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.activity")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.activityType")}</Text>
        <TextInput
          style={styles.input}
          placeholder={t("health.activityPlaceholder")}
          placeholderTextColor={colors.textSecondary}
          value={type}
          onChangeText={setType}
        />

        <Text style={styles.label}>{t("health.activityDuration")}</Text>
        <TextInput
          style={styles.input}
          placeholder="30"
          placeholderTextColor={colors.textSecondary}
      ...
      <Text style={styles.sectionTitle}>{t("health.activityHistory")}</Text>
      {records.length === 0 ? (
        ...
        records.map((record) => (
          <View key={record.id} style={styles.recordCard}>
            <View style={styles.recordHeader}>
              <View style={{ flex: 1 }}>
                <Text style={styles.recordValue}>
                  {record.notes?.split("\n\n")[0] || t("health.activity")} - {record.value} {record.unit}
                </Text>
            ...
            {record.notes?.split("\n\n")[1] && (
              <Text style={styles.recordNotes}>{record.notes.split("\n\n")[1]}</Text>
            )}
          </View>
        ))
      )}
