...
export default function SleepScreen() {
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("sleep", user?.id);
  ...
  const handleSave = async () => {
    if (!hours) {
      Alert.alert(t("common.error"), t("health.sleepHours"));
      return;
    }
    ...
    try {
      ...
      await saveHealthMetric({
        ...
        unit: t("health.hours"),
        notes: `${quality ? `${t("health.sleepQuality")}: ${quality}\n\n` : ""}${notes || ""}`,
        date: today,
      });
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.sleep")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.sleepHours")}</Text>
        <TextInput
          style={styles.input}
          placeholder="8"
        ...
        <Text style={styles.label}>{t("health.sleepQuality")} ({t("common.optional")})</Text>
        <TextInput
          style={styles.input}
          placeholder={t("health.sleepQualityPlaceholder")}
          placeholderTextColor={colors.textSecondary}
          value={quality}
      ...
      <Text style={styles.sectionTitle}>{t("health.sleepHistory")}</Text>
      {records.length === 0 ? (
