...
export default function TemperatureScreen() {
  ...
  const loadRecords = async () => {
    try {
      const data = await getHealthMetrics("temperature", user?.id);
  ...
  const handleSave = async () => {
    if (!temperature) {
      Alert.alert(t("common.error"), t("health.enterTemperature"));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle}>{t("health.temperature")}</Text>
        <View style={{ width: 24 }} />
      </View>

      <View style={styles.form}>
        <Text style={styles.label}>{t("health.temperature")} ({t("health.temperatureUnit")})</Text>
        <TextInput
          style={styles.input}
          placeholder="36.6"
